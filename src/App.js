...
export default function App() {
  ...
  const [copyButtonText, setcopyButtonText] = useState("Share Link")
        ...
        <div style={styles.container}>
          ...
          <div style={styles.controlTools}>
            ...
            <button style={styles.controlButton}
              onClick={() => {
                if (selectedFile) {
                  navigator.clipboard.writeText(`https://ronit-multimedia-app.web.app${selectedFile.path}`)
                  setcopyButtonText("Link copied")
                  setTimeout(
                    function() {
                      setcopyButtonText("Share Link")
                    }
                    .bind(this),
                    3000
                );
                ...
                if (file.path.slice(0, filePath.length) === filePath) {
                  return (
                    <div style={styles.file} className="files" key={file.id} onClick={() => {
                      ...
                      <p>                      <i class={`${selectedFile ? ( (selectedFile.id == file.id && isStar) ? ('fa-solid') : ('fa-regular') ) : ('fa-regular')} fa-star`} ></i>
 {file.name}</p>
                    </div>
                  )
                }
